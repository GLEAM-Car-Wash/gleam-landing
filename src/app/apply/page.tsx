      ...
      <section className="relative pt-40 pb-32 px-6 md:px-12 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="max-w-3xl">
            ...
            >
              No boss. No office. No marketing. Just you, your skills, and guaranteed customers delivered daily.
            </motion.p>
          ...
          >
            {benefits.map((b, idx) => (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-2xl bg-[#0a0a0a] border border-white/[0.06] hover:border-[#E23232]/30 transition-all duration-500 p-8"
              >
                <div className="h-[2px] w-full bg-gradient-to-r from-[#E23232]/40 via-[#E23232] to-[#E23232]/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 absolute top-0 left-0" />
                <div className="flex items-start justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-[#E23232]/[0.08] group-hover:bg-[#E23232]/20 flex items-center justify-center transition-all duration-500">
                    <b.Icon className="w-6 h-6 text-[#E23232]/60 group-hover:text-[#E23232] transition-all duration-500" />
                  </div>
                  <span className="font-display text-2xl text-[#E23232]">{b.highlight}</span>
                </div>
                <h3 className="font-display text-2xl uppercase mb-2 group-hover:text-[#E23232] transition-colors duration-300">{b.title}</h3>
                <p className="font-mono text-[11px] text-white/50 tracking-wider leading-[1.8]">{b.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
          ...
          >
            {testimonials.map((t, idx) => (
              <motion.div
                key={idx}
                variants={benefitVariants}
                className="relative rounded-2xl bg-[#0a0a0a] border border-white/[0.06] p-8"
              >
                {/* Stars */}
                <div className="flex gap-1 mb-4">
                  {[...Array(t.rating)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 text-[#E23232] fill-[#E23232]" />
                  ))}
                </div>

                {/* Quote */}
                <p className="font-serif italic text-white/80 leading-relaxed mb-6">&ldquo;{t.quote}&rdquo;</p>

                {/* Author */}
                <div className="flex items-start justify-between pt-6 border-t border-white/10">
                  <div>
                    <div className="font-display text-lg uppercase mb-1">{t.name}</div>
                    <div className="font-mono text-[10px] text-white/40 uppercase tracking-wider">{t.role}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-display text-2xl text-[#E23232]">{t.earnings}</div>
                    <div className="font-mono text-[9px] text-white/30 uppercase tracking-wider">Avg earnings</div>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-24 px-6 md:px-12 border-t border-white/10">
        <div className="max-w-[1200px] mx-auto text-center">
          <motion.h2
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.7, ease }}
            className="text-4xl md:text-6xl font-display uppercase leading-[0.9] mb-16"
          >
          ...
          >
            {[
              { num: '01', title: 'Apply Online', desc: 'Fill out a quick 5-step form. Takes about 5 minutes.' },
              { num: '02', title: 'We Review', desc: 'Our team reviews your application within 48 hours.' },
              { num: '03', title: 'Start Earning', desc: 'Get approved, get trained, and start taking bookings.' },
            ].map((step, idx) => (
              <motion.div
                key={idx}
                variants={benefitVariants}
              ...
              >
                <span className="font-display text-6xl text-[#E23232]/20 block mb-4">{step.num}</span>
                <h3 className="font-display text-2xl uppercase mb-3">{step.title}</h3>
                <p className="font-mono text-[11px] text-white/50 tracking-wider">{step.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
